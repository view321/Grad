# Grad — the engineering record

What was built, what it refuses to do, and why each decision went the way it
did. [`README.md`](README.md) is the front door and says what Grad is; this file
is the part that starts mattering once you are in the code, and it is the one to
read before changing a gate, a ledger format, or anything that spends money.
...
record. Where this file and the handoffs disagree, the handoffs are the intent
...
release tag, the commit, and whether the checkout was modified). The claim in
the table above is that every number in a report traces to a run record; that is
only complete if the record says which code produced it.
