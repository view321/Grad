...
- `notes/funnel/<date>-<slug>.md` — the full prompt and raw response for both
  Haiku funnel stages, per query (token counts go to `ledger/quota.jsonl`).
  Stages 0 and 3 are the one place this system uses subagents, and this log is
  the mitigation that makes the exception acceptable: debugging a funnel whose
  middle is invisible is guesswork.
- `notes/funnel/<date>-<slug>.json` — the machine-readable trace the UI's funnel
  view renders (300 → 50 → 15, with the reason each survivor was kept).
